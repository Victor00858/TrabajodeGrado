...
PerCtrl.getCVariables = () => {
    try {
        const tank = CTanks.findOne({ idTank: 'Tanque_1' });
        if (!tank) {
...
PerCtrl.CalculatedPerdidas = async () => {
    ...
    const data = await PerCtrl.getHVariables();
    const { hn, ge, ht} = data;
    const hourlyValues = {};
    ...
    function AlturaProm_Hl(Hs, hn, Hlxp, Hlnp, ge, gnx, gnn) {
        let Hl;
        if (hn) {
            Hl = (Hlxp + Hlnp) / 2;
        } else if (ge) {
   ...
   /*  Hl = AlturaProm_Hl(Hs, hn, Hlxp, Hlnp, ge, gnx, gnn);
 */
    function DeltaPb(TC, PR, Pbx, Pbn) {
        let DPb;
        if (TC) {
            DPb = 0;
        } else {
   ...
   /*  Pbxi = Pbxis(PR, Pbx); */
    ...
    function resta_Hl(Hn, Hlxp, Hlnp, gnx, gnn) {
        let Hlm;
        if (Hn) {
            Hlm = (Hlxp - Hlnp);
        } else {
   ...
   /*  Hlm = resta_Hl(Hn, Hlxp, Hlnp, gnx, gnn); */
    ...
    /* for (const hora in data) {
        if (hora.startsWith("hora_")) {
            const { Hlxp, Hlnp, tbx, tbn, Q, Pbx, Pbn } = data[hora];
            ...
            hourlyValues[hora] = calculatedValues;
        }
    }
...
const hourlyValues = PerCtrl.CalculatedPerdidas();
